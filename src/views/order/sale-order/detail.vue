...
import { saleOrderOrderDetail } from '@/api/order'
...
export default class extends Vue {
  ...
  private async getsaleOrderOrderDetail() {
    this.listLoading = true
    const { id: saleOrderId } = this.params
    const { payload, code } = await saleOrderOrderDetail({ saleOrderId }).finally(() => { this.listLoading = false })
    if (code === '0') {
